...
from grabnet.data.oishape_dataset import OIShape
...
class Trainer:
    ...
    def __init__(self, cfg, inference=False):
        ...
        self.use_amp = use_cuda  # 只在 GPU 上启用 AMP
        if self.use_amp:
            self.scaler_cnet = torch.cuda.amp.GradScaler()
            self.scaler_rnet = torch.cuda.amp.GradScaler()
            logger('Enabled Automatic Mixed Precision (AMP) training')
        else:
            self.scaler_cnet = None
            self.scaler_rnet = None
            logger('Training with full precision (float32)')
    ...
    def load_data(self, cfg, inference):
        ...
        ds_test = OIShape(split='test', bps_path='grabnet/configs/bps.npz', cache_bps=True)
        self.data_info[ds_name]['frame_names'] = [f"test_{i}" for i in range(len(ds_test))]
        self.data_info[ds_name]['frame_sbjs'] = ['oakink'] * len(ds_test)
        self.ds_test = DataLoader(ds_test, batch_size=cfg.batch_size, shuffle=False, drop_last=True)

        if not inference:
            ds_name = 'train'
            self.data_info[ds_name] = {}
            ds_train = OIShape(split='train', bps_path='grabnet/configs/bps.npz', cache_bps=True)
            self.data_info[ds_name]['frame_names'] = [f"train_{i}" for i in range(len(ds_train))]
            self.data_info[ds_name]['frame_sbjs'] = ['oakink'] * len(ds_train)
            self.ds_train = DataLoader(ds_train, **kwargs)

            ds_name = 'val'
            self.data_info[ds_name] = {}
            ds_val = OIShape(split='val', bps_path='grabnet/configs/bps.npz', cache_bps=True)
            self.data_info[ds_name]['frame_names'] = [f"val_{i}" for i in range(len(ds_val))]
            self.data_info[ds_name]['frame_sbjs'] = ['oakink'] * len(ds_val)
            ...
            self.data_info['hand_vtmp'] = None
            self.data_info['hand_betas'] = None

        self.bps = ds_test.bps_basis if hasattr(ds_test, 'bps_basis') else None
        self.n_obj_verts = ds_test.n_samples
    ...
    def _select_key(self, dorig, candidates):
        """从 dorig 里按候选名顺序取第一个存在的键。"""
        for k in candidates:
            if k in dorig:
                return k, dorig[k]
        return None, None

    def _build_rnet_input(self, dorig, it=None, phase='train'):
        """
        按 RefineNet.forward 的签名，从 dorig 里取到 required args，
        并额外派生/补充 RefineNet 常用的 kwargs（quat/object embedding 输入）。
        """
        rnet = self._get_rnet_model()
        sig = inspect.signature(rnet.forward)
        ...
        for name, param in sig.parameters.items():
            ...
            _, v = self._select_key(dorig, cands)
            if v is None:
        ...
        if missing:
            have = sorted(list(dorig.keys()))
            msg_lines = [
                f"RefineNet 输入缺失（phase={phase} it={it}）：",
                *[f"  - 需要参数 `{n}`，尝试键名 {cands} 都没找到" for n, cands in missing],
                f"dorig 当前包含的 keys（{len(have)} 个）：",
                "  " + ", ".join(have[:80]) + (" ..." if len(have) > 80 else ""),
            ]
            raise KeyError("\n".join(msg_lines))

        # 2) 关键输入转 float32/contiguous
        for k in ['h2o_dist', 'fpose_rhand_rotmat_f', 'trans_rhand_f', 'global_orient_rhand_rotmat_f', 'verts_object']:
            if isinstance(rnet_input.get(k, None), torch.Tensor):
                rnet_input[k] = rnet_input[k].contiguous().float()

        # ✅ 2.5) 关键修复：把扁平 rotmat 还原成 (B,*,3,3)
        # RefineNet.forward 里会对 rotmat 做 [..., :2]，因此必须是 (...,3,3) 结构
        bs = rnet_input['h2o_dist'].shape[0]

        fp = rnet_input.get('fpose_rhand_rotmat_f', None)
        if isinstance(fp, torch.Tensor):
            # 期望: (B, 15, 3, 3)
            if fp.dim() == 2 and fp.shape[1] == 135:
                rnet_input['fpose_rhand_rotmat_f'] = fp.view(bs, 15, 3, 3).contiguous()
            elif fp.dim() == 4 and fp.shape[-2:] == (3, 3):
                # 已经是 rotmat
                pass
            else:
                raise ValueError(f"Unexpected shape for fpose_rhand_rotmat_f: {tuple(fp.shape)}; expected (B,135) or (B,15,3,3)")

        go = rnet_input.get('global_orient_rhand_rotmat_f', None)
        if isinstance(go, torch.Tensor):
            # 期望: (B, 3, 3)
            if go.dim() == 2 and go.shape[1] == 9:
                rnet_input['global_orient_rhand_rotmat_f'] = go.view(bs, 3, 3).contiguous()
            elif go.dim() == 3 and go.shape[-2:] == (3, 3):
                pass
            else:
                raise ValueError(f"Unexpected shape for global_orient_rhand_rotmat_f: {tuple(go.shape)}; expected (B,9) or (B,3,3)")

        # 3) ✅ 额外补充 kwargs：bps_object（常用于 obj embedding）
        if 'bps_object' in dorig and isinstance(dorig['bps_object'], torch.Tensor):
            rnet_input['bps_object'] = dorig['bps_object'].contiguous().float()
            # 多给几个常见别名，避免 forward 用的是别的名字
            rnet_input['obj_bps'] = rnet_input['bps_object']
            rnet_input['bps'] = rnet_input['bps_object']

        # 4) ✅ 额外补充 kwargs：四元数版本（很多实现用 quat 拼接，维度更吻合 877）
        try:
            bs = rnet_input['trans_rhand_f'].shape[0]

            # global orient rotmat(9)->quat(4)
            gor_rm = rnet_input.get('global_orient_rhand_rotmat_f', None)
            if isinstance(gor_rm, torch.Tensor) and gor_rm.dim() == 2 and gor_rm.shape[1] == 9:
                gor_rm_ = gor_rm.view(bs, 3, 3).contiguous()
                gor_q = matrix_to_quaternion(gor_rm_)  # (B,4)
                rnet_input['global_orient_rhand_quat_f'] = gor_q.contiguous().float()
                rnet_input['global_orient_quat_f'] = rnet_input['global_orient_rhand_quat_f']

            # fpose rotmat(135)->(B,15,3,3)->quat(B,15,4)->(B,60)
            fpose_rm = rnet_input.get('fpose_rhand_rotmat_f', None)
            if isinstance(fpose_rm, torch.Tensor) and fpose_rm.dim() == 2 and fpose_rm.shape[1] == 135:
                fpose_rm_ = fpose_rm.view(bs, 15, 3, 3).contiguous()
                fpose_q = matrix_to_quaternion(fpose_rm_)  # (B,15,4)
                fpose_q_flat = fpose_q.view(bs, 60).contiguous().float()
                rnet_input['fpose_rhand_quat_f'] = fpose_q_flat
                # 常见别名
                rnet_input['hand_pose_quat_f'] = fpose_q_flat
                rnet_input['fpose_quat_f'] = fpose_q_flat

            # （保留你已有的 axis-angle，作为另一种可能）
            if isinstance(fpose_rm, torch.Tensor) and fpose_rm.dim() == 2 and fpose_rm.shape[1] == 135:
                fpose_aa = matrix_to_axis_angle(fpose_rm.view(bs, 15, 3, 3).contiguous())
                rnet_input['fpose_rhand_f'] = fpose_aa.view(bs, 45).contiguous().float()

            if isinstance(gor_rm, torch.Tensor) and gor_rm.dim() == 2 and gor_rm.shape[1] == 9:
                gor_aa = matrix_to_axis_angle(gor_rm.view(bs, 3, 3).contiguous())
                rnet_input['global_orient_rhand_f'] = gor_aa.contiguous().float()

        except Exception as e:
            if it == 0:
                self.logger(f"[WARNING] Failed to derive quat/aa kwargs for RefineNet: {e}")

        # 5) debug
        if it == 0 and phase == 'train':
            self.logger("[DEBUG] RefineNet.forward signature: %s" % str(sig))
            for k, v in rnet_input.items():
                if isinstance(v, torch.Tensor):
                    self.logger(f"[DEBUG] rnet_input[{k}]: shape={tuple(v.shape)} dtype={v.dtype} device={v.device}")
                else:
                    self.logger(f"[DEBUG] rnet_input[{k}]: type={type(v)}")
    ...
    def train(self):
        ...
        for it, dorig in enumerate(self.ds_train):
            ...
            if self.fit_cnet:
                cnet_input = {
                    'bps_object': dorig['bps_object'],
                    'trans_rhand': dorig['trans_rhand'],
                    'global_orient_rhand_rotmat': dorig['global_orient_rhand_rotmat'],
                }

                if self.use_amp:
                    with torch.cuda.amp.autocast():
                        drec_cnet = self.coarse_net(**cnet_input)
                        loss_total_cnet, cur_loss_dict_cnet = self.loss_cnet(dorig, drec_cnet)

                    self.scaler_cnet.scale(loss_total_cnet).backward()
                    self.scaler_cnet.step(self.optimizer_cnet)
                    self.scaler_cnet.update()
                else:
                    drec_cnet = self.coarse_net(**cnet_input)
                    loss_total_cnet, cur_loss_dict_cnet = self.loss_cnet(dorig, drec_cnet)
            ...
            if self.fit_rnet:
                ...
                # ✅ 用 forward 签名自动组装输入（不再手写 dict）
                rnet_input = self._build_rnet_input(dorig, it=it, phase='train')

                if self.use_amp:
                    with torch.cuda.amp.autocast():
                        drec_rnet = self.refine_net(**rnet_input)
                        loss_total_rnet, cur_loss_dict_rnet = self.loss_rnet(dorig, drec_rnet)

                    self.scaler_rnet.scale(loss_total_rnet).backward()
                    self.scaler_rnet.step(self.optimizer_rnet)
                    self.scaler_rnet.update()
                else:
                    drec_rnet = self.refine_net(**rnet_input)
                    loss_total_rnet, cur_loss_dict_rnet = self.loss_rnet(dorig, drec_rnet)
    ...
    def evaluate(self, ds_name='val'):
        ...
        with torch.no_grad():
            for it, dorig in enumerate(data):
                ...
                if self.fit_rnet:
                    ...
                    # ✅ 同样自动组装
                    rnet_input = self._build_rnet_input(dorig, it=it, phase='eval')
    ...
    def params_rnet(self, dorig):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            dorig_verts_rhand_f = dorig['verts_rhand_f'].float()
            dorig_verts_rhand = dorig['verts_rhand'].float()
            dorig_verts_object = dorig['verts_object'].float()

            rh_mesh = Meshes(verts=dorig_verts_rhand_f, faces=self.rh_f).to(self.device).verts_normals_packed().view(-1, 778, 3)
            rh_mesh_gt = Meshes(verts=dorig_verts_rhand, faces=self.rh_f).to(self.device).verts_normals_packed().view(-1, 778, 3)

            o2h_signed, h2o, _ = point2point_signed(dorig_verts_rhand_f, dorig_verts_object, rh_mesh)
            o2h_signed_gt, h2o_gt, _ = point2point_signed(dorig_verts_rhand, dorig_verts_object, rh_mesh_gt)
    ...
    def loss_cnet(self, dorig, drec, ds_name='train'):
        ...
        # ✅ 在 autocast 区域内，强制使用 float32
        with torch.cuda.amp.autocast(enabled=False):
            # 确保所有几何计算使用 float32
            drec_float32 = {k: v.float() if isinstance(v, torch.Tensor) else v for k, v in drec.items()}
            out_put = self.rhm_train(**drec_float32)
            verts_rhand = out_put.vertices  # [B, 778, 3] float32
            ...
            o2h_signed, h2o, _ = point2point_signed(verts_rhand, dorig_verts_object, rh_mesh)
            o2h_signed_gt, h2o_gt, o2h_idx = point2point_signed(dorig_verts_rhand, dorig_verts_object, rh_mesh_gt)
        ...
        ########## verts loss
        loss_mesh_rec_w = 35 * (1. - self.cfg.kl_coef) * torch.mean(torch.einsum('ijk,j->ijk', torch.abs((dorig_verts_rhand - verts_rhand)), self.v_weights))
        ########## edge loss
        loss_edge = 30 * (1. - self.cfg.kl_coef) * self.LossL1(self.edges_for(verts_rhand, self.vpe), self.edges_for(dorig_verts_rhand, self.vpe))
        ########## KL loss
    ...
    def loss_rnet(self, dorig, drec, ds_name='train'):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            rh_mesh = Meshes(verts=verts_rhand, faces=self.rh_f).to(self.device).verts_normals_packed().view(-1, 778, 3)
            h2o_gt = dorig['h2o_gt'].float()
            o2h_signed, h2o, _ = point2point_signed(verts_rhand, dorig_verts_object, rh_mesh)

        ######### dist loss
        loss_dist_h = 35 * (1. - self.cfg.kl_coef) * torch.mean(torch.einsum('ij,j->ij', torch.abs(h2o.abs() - h2o_gt.abs()), self.v_weights2))
        ########## verts loss
        loss_mesh_rec_w = 20 * (1. - self.cfg.kl_coef) * torch.mean(torch.einsum('ijk,j->ijk', torch.abs((dorig_verts_rhand - verts_rhand)), self.v_weights2))
        ########## edge loss
        loss_edge = 10 * (1. - self.cfg.kl_coef) * self.LossL1(self.edges_for(verts_rhand, self.vpe), self.edges_for(dorig_verts_rhand, self.vpe))
        ##########
        ...
        for split, ds in [('val', self.ds_val), ('test', self.ds_test), ('train', self.ds_train)]:
            ...
            with torch.no_grad():
                for it, dorig in enumerate(ds):
                    ...
                    # ✅ 同样自动组装
                    rnet_input = self._build_rnet_input(dorig, it=it, phase=f'eval:{split}')
